...
console.log('funguju!', episodes);

const renderPodcasts = (episodes) => {
  const podcastList = document.querySelector('.episodes-list');
  podcastList.innerHTML = episodes.map((episode) => Podcast({num: episode.num, title: episode.title, guest: episode.guest })).join('');
};
